...
const BuilderHeader = ({
  ...
  useEffect(() => {
    ...
    const updateFields = () => {
      console.log(
        "updateFields galleryDesc, exportData, galleryId, id",
        galleryDesc,
        exportData,
        galleryId,
        id
  ...
  const onFinish = (values) => {
    ...
    console.log(
      "save values to",
      "users/" + uidToSaveTo + "/galleryData/" + id
    );
    const options = {
  ...
  const getPublicUrl = () => {
    if (nameEncoded || galleryDesc.nameEncoded) {
      return (
        "https://hangar.social/gallery/" + nameEncoded ||
        galleryDesc.nameEncoded
      );
    }
  };
              ...
              <DetailsDropdown detailsOpen={detailsOpen} setCenter={location}>
                <Row gutter={8}>
                  <Col span={16}>
                    ...
                    <Form.Item>
                      <MapModal
                        locationSet={setLocationCallback}
                        setCenter={location}
                      />
                  ...
                  <Col span={8} style={{ textAlign: "center", padding: 10 }}>
                    <div onClick={selectArt}>
                      ...
                      <Button style={{ margin: "auto" }} loading={selectingArt}>
                        {selectingArt ? "Cancel" : "Select Featured Image"}
                      </Button>
                      {selectingArt ? (
                        <div>Select a work from Vault</div>
                      ) : (
    ...
    <Route
      ...
      render={(routeProps) => {
        ...
        return (
          ...
          >
            Visit
            <ArrowRightOutlined key="list-vertical-star-o" style={{}} />
          </Button>
        );
      }}
    />
...
export default compose(withRouter, withFirebase)(BuilderHeader);
