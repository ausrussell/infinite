...
const GalleryList = ({
  ...
  const [springProps, setSpringProps, stopSpringProps] = useSpring(() => ({
    scroll: 1,
  }));
  const { getList, getAssetOnce } = firebase;
  const [loadedCounter, setLoadedCounter] = useState(0);
  // useEffect(() => {
  //   console.log("useEffect empty array", galleriesList);
  //   if (onPage) {
  //     galleriesList.forEach((item) => {
  //       item.ref = createRef();
  //       galleriesRefs.current.push(item.ref);
  //     });
  //   }
  // }, [onPage]);
  useEffect(() => {
        ...
        data.forEach((childSnapshot) => {
          ...
          if (snap.art) {
            ...
            const newPath =
              ...
              parts[1] +
              "/art/" +
              snap.art[Math.floor(Math.random() * snap.art.length)];
            const options = {
              refPath: newPath,
              once: true,
              extras: snap,
  ...
  selectedListItem.new &&
    selectedListItem.new.current &&
    setSpringProps({ scroll: selectedListItem.new.current.offsetTop });

  useEffect(() => {
    const doSelect = () => {
      // console.log(
      //   "galleriesRefs.current,galleriesRefs",
      //   markerSelected,
      //   galleriesRefs.current,
      //   galleriesRefs
      // );
      const markerSelected2 = galleriesRefs.current.filter(
        (item) => item.current.id === markerSelected
      );
  ...
  useEffect(() => {
    if (oldSelectedListItem) {
      oldSelectedListItem.current.classList.remove(
  ...
  const onContainerClickHandler = (e, item, ref) => {
    ...
    console.log("onContainerClickHandler")
    if (
      e.target.src ||
      ...
      <List
        ...
        renderItem={(item) => {
          return (
            <GalleryListItem
              item={item}
              artLoadedListCallback={artLoadedListCallback}
              onClickHandler={onClickHandler}
              onContainerClickHandler={onContainerClickHandler}
              ref={item.ref}
            />
          );
        }}
...
const GalleryListItem = React.forwardRef((props, ref) => {
  ...
  const {
    ...
    artLoadedListCallback,
  } = props;
  const galleryImg = item.galleryImg
    ? item.galleryImg.thumb || item.galleryImg.url
    : galleryPlaceholder;
  if (item.call && !art) {
    item.call.then((snap) => {
    ...
    >
      <List.Item
        ...
        actions={[
          <span
            className="icon-link"
            onClick={() => onClickHandler("Locate", item, ref)}
          >
            <EnvironmentOutlined
          ...
          <Route
            ...
            render={(routeProps) => {
              ...
              return (
                <span
                  className="icon-link"
                  onClick={() => onClickHandler("Visit", routeProps)}
                >
                  <ArrowRightOutlined
                    key="list-vertical-star-o"
                    style={{ marginRight: 8 }}
      ...
      >
        <List.Item.Meta
          ...
          description={
            <div>
              {item.userDisplayName && <p>Built by: {item.userDisplayName}</p>}
...
export default compose(withRouter, withFirebase)(GalleryList);
