...
class GalleryBase extends Component {
  ...
  setScene() {
    ...
    // if (this.state.galleryData) {createSceneData(galleryData);}
    // const options = {
    //   scene: this.scene,
  ...
  mainSceneCallback = (data) => {
    console.log("mainSceneCallback", data);
  };
...
const mapStateToProps = (state) => {
  ...
  console.log("{ camera, scene, renderer }",camera, scene, renderer)
  console.log("Gallery state ", camera, state.scene.children);
  // console.log("state.sceneData", state.sceneData);
  const collidableObjects = state.scene.children.filter(item => item.name === "wall");
  console.log("collidableObjects in mapState",collidableObjects)
const art =[];
  state.scene.traverse(function (node) {
      console.log("art?",node.name);
      if (node.name === "art"){
        // collidableObjects.push(node)
        console.log("art yes?",node);
art.push(node)
      }
    })

  //   const gotArt = collidableObjects[i].getArt();
  //   if (gotArt.length) {
  //     gotArt.forEach(item => {
  ...
  //   }
  // }
  return { camera, scene: state.scene, renderer, collidableObjects, art }; //{scene: state.scene};
};

const Gallery = compose(
  connect(mapStateToProps, { createSceneData }),
  withAuthentication,
  withFirebase,
  withRouter
)(GalleryBase);
